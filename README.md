...
Select a Git branch to use:
    ...
    bin/setup master  # template + basics + my flair + dvorak + new stuff
...
``<Space>.c``         | (NERDCommenter) invert comment markers
``<Space>.C``         | (NERDCommenter) toggle comment markers
``<Space>.o``         | (fzf) open file from history
``<Space>//``         | fuzzy-regex search in buffer
``<Space>/a``         | (ALE) Search for symbols in the workspace.
...
``<Space>fb``         | (ALE) Fix problems with the current buffer.
...
``<Space>g*``         | (ALE) Find references for the word under the cursor.
``<Space>ga``         | (ALE) Print information about the symbol at the cursor.
``<Space>gA``         | (ALE) Show full linter message for problem nearest to cursor.
``<Space>gd``         | (ALE) Open the definition of the symbol under the cursor.
``<Space>gD``         | (ALE) Open the definition of the symbol's type under the cursor.
...
``[C``                | (ALE) Go to first warning or error in the current buffer.
``[c``                | (ALE) Go to previous warning or error in the current buffer.
...
``]C``                | (ALE) Go to last warning or error in the current buffer.
``]c``                | (ALE) Go to next warning or error in the current buffer.
